...
bot.on("message", async (ctx) => {
    ...
    if (isSettling) {
      ...
      if (isSettled) {
        ...
        ctx.reply(`Отлично, ${name}, вот контакт человека, который тебя примет: ${info}. Обязательно свяжись с сопровождающим тебя человеком заранее, чтобы обсудить детали, доброй ночи :)`, getMainMenu());
      } else {
        await ctx.reply(`Извини, я не нашел тебя в базе, но не переживай, найди кого-нибудь с бейджиком или подойди на регистрацию и тебе обязательно помогут`, getMainMenu());
      }
    } else {
